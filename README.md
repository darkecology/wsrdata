...
You may render an array for a radar scan interactively, if you have not obtained rendered arrays already.
...
- This wsrdata repo renders arrays with `ydirection='xy'` and save annotations in json files 
in the **geographical direction**.
...
This is no longer needed if you read radar scans using the read_http function instead of read_s3 in 
    ...
    - **annotations** is for different versions of annotations; 
      **user_models** is to normalize annotator style differences
        - Given different annotation formats, related code in step 6 of 
          **dataset_preparation/prepare_dataset_\*.py** will need to be updated.
...
- **tools** contains various helper scripts and notebooks:
    - **visualization.ipynb** can visualize interactively render an array or take a pre-rendered array, 
    and visualize it with its annotation(s) from a json file.
    - **visualization.py** generates png images that visualize selected channels in rendered arrays for 
    a given list of scans with annotations from a json file.
...
- Prepare a dataset
  ...
  `dataset_preparation/prepare_dataset_v0.1.0_help` contains optional helper functions that are useful for accelerating 
...
- annotations
  ...
  for annotator information.
    ...
    These user factors are manually imported to `dataset_preparation/prepare_dataset_v0.0.1.py`.
- arrays
