...
DESCRIPTION         = "The wsrdata roost dataset v0.2.0 with bbox annotations."
...
URL                 = ""
PRE_DATASET_VERSION = ["v0.1.0"] # Load scans and annotations from some previous dataset version(s) to begin with
...
for pre_dataset_version in PRE_DATASET_VERSION:
    assert os.path.exists(f"{DATASET_DIR}/roosts_{pre_dataset_version}.json")
    pre_dataset = json.load(open(f"{DATASET_DIR}/roosts_{pre_dataset_version}.json", 'r'))
    ...
    if ANNOTATION_VERSION:
        ...
        for csv_file in csv_files:
            ...
            for annotation in annotations:
                ...
                if annotation[18] != 'true' or annotation[20] not in [
                    "swallow-roost", "weather-roost", "unknown-noise-roost", "AP-roost", "bad-track"
                ]:
                    continue
                ...
                new_annotation = {
                    ...
                    "notes":            annotation[22],
                    "day_notes":        annotation[23], # miss, weather, ap, pap
                }
    ...
    for pre_dataset_version in PRE_DATASET_VERSION:
        ...
        for scan in pre_dataset["scans"]:
            dataset["scans"].append({
                ...
                "dataset_version":      pre_dataset["info"]["dataset_version"],
                "key":                  scan["key"],
                "minutes_from_sunrise": scan["minutes_from_sunrise_dict"],
